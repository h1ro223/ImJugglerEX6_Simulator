...
class Reel {
  ...
  render() {
    this.strip.style.transform = `translate3d(0, ${(-this.pos * this.cellH).toFixed(2)}px, 0)`;
...
/* ================= リール絵柄の描画 (初期の見た目) ================= */
/* 画像を「横幅いっぱい・縦は中央寄せ」で16:9セルに描画する。
   - 元の1280x720画像ならセル全面にそのまま収まる
   - トリミング済みの1280x470画像でも上下に余白がつき、初期と同じ見た目になる
   512x288(16:9)に縮小してGPU負荷も削減 */
...
function layoutReels() {
  ...
  const cellH = Math.round(w * 9 / 16); // 元の16:9セル比率
...
function bindEvents() {
  ...
  document.addEventListener('touchmove', e => {
    if (e.target.closest && (e.target.closest('#modal') || e.target.closest('#payModal'))) return;
  ...
  document.addEventListener('touchend', e => {
    if (e.target.closest && (e.target.closest('#modalOverlay') || e.target.closest('#payOverlay'))) { lastTouchEnd = 0; return; }
    const t = Date.now();
