...
/* ① 画像の白余白を自動検出して絵柄部分(バウンディングボックス)だけを切り出す
   ② 実機のように絵柄を横に大きく引き伸ばしてセルへ描画 (図柄ごとに目標幅を設定)
   ③ 512x188(セルと同比率)に縮小してGPU負荷も削減 */
const SYM_FIT_W = { 1: 0.86, 2: 0.86, 3: 0.90, 4: 0.88, 5: 0.90, 6: 0.985, 7: 0.985 };
const SYM_FIT_H = 0.97;
  ...
  for (const sym in SYM_IMG) {
    ...
    src.onload = () => {
      try {
        ...
        /* 絵柄を目標サイズへ引き伸ばして中央に描画 (実機風) */
        const fitW = (SYM_FIT_W[sym] || 0.88) * W;
        const fitH = SYM_FIT_H * H;
        const cv = document.createElement('canvas');
        cv.width = W; cv.height = H;
        const c = cv.getContext('2d');
        c.fillStyle = '#fff';
        c.fillRect(0, 0, W, H);
        c.drawImage(src, minX, minY, maxX - minX, maxY - minY,
