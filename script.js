...
/* 1280x720の原寸画像を126セルにそのまま敷き詰めるとスマホのGPU負荷が高くラグの原因に
...
/* --- レバーON --- */
/* --- レバーON (自動BET時はMaxBet音とLever音の被り防止で1秒遅延) --- */
function leverOn() {
...
function resolveGame() {
  ...
  if (bonusAligned) {
    ...
    if (pay > 0) {
      ...
      if (hasGrape || hasBell) {
        const key = pay >= 14 ? 'GRAPE14' : 'GRAPE8';
        audio.playSE(key);
        payoutSndMs = audio.duration(key, 900);
      } else {
        audio.get1Loop(pay); // ピエロ・チェリー等: (枚数-1)回ループ後にGet1Finish
        payoutSndMs = Math.max(0, pay - 1) * 110 + audio.duration('GET1FIN', 500);
      }
...
function autoNextGame(delayMs) {
  autoSchedule(() => {
    ...
    leverOn(); // 自動MAXBET→1秒→レバー (リプレイ時はそのままレバー)
  }, delayMs);
}
...
function saveGame() {
  try {
    const data = {
      ...
      assist: state.assist, easyMode: state.easyMode,
