...
/* ================= リール絵柄の実機風最適化 ================= */
/* ① 画像の白余白を自動検出して絵柄部分(バウンディングボックス)だけを切り出す
   ② 絵柄本来の縦横比を保ったまま「縦がセルの97%」になるよう拡大 (歪ませない)
      横長の図柄(7/BAR)は横98.5%を上限にして収める
   ③ 512x188(セルと同比率)に縮小してGPU負荷も削減 */
const SYM_FIT_H = 0.97;   // 縦の目標割合
const SYM_MAX_W = 0.985;  // 横の上限割合

function optimizeSymbolImages() {
  const W = 512, H = 188; // 1280x470と同比率
  for (const sym in SYM_IMG) {
    const src = new Image();
    src.onload = () => {
      try {
        /* 白(に近い)ピクセル以外のバウンディングボックスを検出 */
        ...
        if (maxX <= minX || maxY <= minY) { minX = 0; minY = 0; maxX = tmp.width; maxY = tmp.height; }
        const bw = maxX - minX, bh = maxY - minY;
        /* アスペクト比を保ったまま拡大: 縦97%基準、横は98.5%を上限 */
        const scale = Math.min((SYM_FIT_H * H) / bh, (SYM_MAX_W * W) / bw);
        const fitW = bw * scale, fitH = bh * scale;
        const cv = document.createElement('canvas');
        cv.width = W; cv.height = H;
        const c = cv.getContext('2d');
        c.fillStyle = '#fff';
        c.fillRect(0, 0, W, H);
        c.drawImage(src, minX, minY, bw, bh,
                    (W - fitW) / 2, (H - fitH) / 2, fitW, fitH);
